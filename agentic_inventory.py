...
# --- 2. DATABASE ---
DB_FILE = 'aroha_ultimate_v115.db'
...
if st.session_state.page == "Dashboard":
    ...
    st.markdown("<div class='insight-box'><b>💡 User Insight:</b> All nodes synchronized. Open <b>PREKSHA</b> for profiles or <b>STAMBHA</b> for risk testing.</div>", unsafe_allow_html=True)
...
# STAMBHA
    ...
    if not df.empty:
        ...
        st.table(pd.DataFrame([{"Asset": p['name'], "TTS (d)": round(p['current_stock']/12,1), "Status": "Safe" if (p['current_stock']/12) > (p['lead_time']*(3 if "Port" in s else 1)) else "CRITICAL"} for _,p in df.iterrows()]))
