...
# --- 1. PREMIUM UI CONFIG (SPECTRUM COLORS, WATERMARK & RESPONSIVITY) ---
st.set_page_config(
...
def apply_aroha_style():
    # Double braces {{ }} used for CSS to prevent Python f-string SyntaxError
    st.markdown(f"""
        <style>
...
# --- 5. TOP HUD ---
st.markdown(f"<div class='ticker-wrap'><div class='ticker-text'>[DHWANI] Neural link active for {st.session_state.user.upper()} // [LOGISTICS] Port congestion detected in SE Asia // [SENSING] Predicted +12% demand spike.</div></div>", unsafe_allow_html=True)
...
# PREKSHA
    ...
    if df.empty: st.warning("Add data in Nyasa.")
        ...
        with col_v:
            ...
            st.plotly_chart(px.area(y=preds, title="AI Forecasting Path", template="plotly_dark").update_traces(line_color='#7F00FF'), use_container_width=True)
            st.markdown(f"<div class='ai-decision-box'><h3>🤖 AI DECISION</h3>Reorder <b>{max(0, preds.sum() - p_row['current_stock'])}</b> units now.</div>", unsafe_allow_html=True)
...
# KARMA (Restored Tasks & Missions)
elif st.session_state.page == "Karma":
    st.markdown("<div class='feature-header' style='color:#FF33FF !important;'>KARMA</div>", unsafe_allow_html=True)
    t1, t2 = st.tabs(["🏹 Missions", "🧠 Performance"])
    with t1:
        st.markdown("<div class='saas-card'><b>Current Assignment:</b> Pick 12x Chassis Station B1<br>Priority: High</div>", unsafe_allow_html=True)
        st.warning("⚠️ Fatigue Alert: Picking speed decreasing. Suggest 10min break.")
    with t2:
...
# VITTA (Restored metrics + Pie)
    ...
    if not df.empty:
        ...
        with c1:
            ...
            st.markdown(f"<div class='financial-stat' style='margin-top:20px; color:red;'>Idle Capital Risk<br><h2>₹{v*0.15:,.0f}</h2></div>", unsafe_allow_html=True)
        with c2: st.plotly_chart(px.pie(df, values='current_stock', names='name', hole=0.5, template="plotly_dark"))

# SANCHARA (Restored map + Description + Returns)
    ...
    t1, t2 = st.tabs(["🌐 Precision Map", "↩️ Returns (PUNAH)"])
    with t1:
        map_points = pd.DataFrame({'lat':[12.97, 22.31, 1.35], 'lon':[77.59, 114.16, 103.81], 'Node':['Hub','Factory','Port'], 'Address':['Bangalore','HK','Singapore (🔴 CLOSED)']})
        ...
        st.markdown("<div class='insight-box'>🗺️ <b>Strategic Guide:</b> Hover markers for exact physical addresses. Red zones indicate active disruptions.</div>", unsafe_allow_html=True)
    with t2:
        st.write("### Detailed Returns Log")
        st.table(pd.DataFrame({'Product':['Laptop','Monitor'], 'Amount':[4,2], 'Reason':['Logic Failure','Screen Bleed']}))
...
# SAMVADA & MITHRA+
    ...
    if not df.empty:
        ...
        if st.button("🚀 NEGOTIATE"):
            st.info(f"AI drafted strategy for {vendor}. Target: 10% unit cost reduction.")
