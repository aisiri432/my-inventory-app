...
# --- 1. PREMIUM UI CONFIG (UNIVERSAL RESPONSIVE & RADIANT WATERMARK) ---
st.set_page_config(page_title="AROHA | Strategic Intelligence", layout="wide", page_icon="💠", initial_sidebar_state="auto")
...
# --- 5. TOP HUD TICKER ---
st.markdown(f"<div class='ticker-wrap'><div class='ticker-text'>[DHWANI] {st.session_state.user.upper()} ACTIVE // [LOGISTICS] Port congestion reported in SE Asia // [MARKET] Sentiment sensors tracking +12% Demand // [VOICE] Samvada Node Listening.</div></div>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    nodes = [
        ...
        ("👷‍♂️ KARMA", "Karma", "Performance Logic"),
...
# 🏠 DASHBOARD (Neural Pulse & Notifications)
if st.session_state.page == "Dashboard":
    ...
    <span style='color:#00D4FF;'>● NEURAL LINK: <b>STABLE</b></span> | <span>SYNC: <b>98%</b></span></div>""", unsafe_allow_html=True)

    c1, c2, c3 = st.columns(3)
    with c1: st.metric("📝 Assets Managed", len(df))
    with c2: st.metric("💰 Treasury Value", f"₹{val:,.0f}")
    with c3: st.metric("🛡️ System Health", "OPTIMAL")
    
    with st.expander("🔔 CHETANA: Critical Strategic Directives", expanded=True):
        st.info("DIRECTIVE: AI sensing +12% Demand for next weekend. Re-calibration required in Preksha.")
        if val > 100000: st.error("RISK: High Capital Lock-in detected in electronics category.")
...
# 📈 PREKSHA (PROFILES + REVIEWS + AI SUGGESTIONS)
    ...
    if df.empty: st.warning("Add data in NYASA node.")
        ...
        with col_v:
            ...
            Recommendation: Order <b>{max(0, total_needed - p_row['current_stock'])}</b> units immediately.</div>""", unsafe_allow_html=True)
...
# 🛡️ STAMBHA (FIXED RISK MESSAGES)
    ...
    s_scenario = st.selectbox("Trigger Shock", ["Normal", "Port Closure (3x Lead)", "Factory Fire (+30d)"])
    df = get_user_data()
    if not df.empty:
        for _, p in df.iterrows():
            ttr = p['lead_time'] * (3 if "Port" in s_scenario else 1)
            tts = round(p['current_stock'] / 12, 1)
            if tts < ttr: st.error(f"🔴 CRITICAL RISK: {p['name']} stockout in {tts}d. Recovery: {ttr}d. ACTION REQUIRED.")
        st.table(df[['name', 'current_stock', 'lead_time']])

# 👷‍♂️ KARMA (SKILL RADAR + FATIGUE)
elif st.session_state.page == "Karma":
    st.markdown("<div class='feature-header'>👷‍♂️ KARMA</div>", unsafe_allow_html=True)
    t1, t2 = st.tabs(["🏹 Missions", "🧠 Intelligence Radar"])
    with t1:
        st.markdown("<div class='saas-card'><b>Task Assigned:</b> Pick 12x Titanium Chassis for station B.<br><b>Status:</b> High Priority</div>", unsafe_allow_html=True)
        st.warning("⚠️ Fatigue Alert: Picking speed decreasing in Aisle 4. Suggest team rotation.")
    with t2:
        fig = go.Figure(data=go.Scatterpolar(r=[90, 95, 70, 85, 60], theta=['Picking','Accuracy','Packing','Safety','Speed'], fill='toself'))
...
# 📦 SANCHARA (MAP + ADDRESSES + SHIPPED)
    ...
    with t1:
        ...
        st.markdown("<div style='background:rgba(255,255,255,0.02); padding:15px; border-radius:10px; border:1px solid #333; margin-top:15px;'><h4 style='color:#00D4FF; margin:0;'>🗺️ Strategic Map Guide</h4><p>📍 Blue: Hubs. 🔴 Red: Global Crisis points. Hover dots for addresses.</p></div>", unsafe_allow_html=True)
    with t2:
        c1, c2 = st.columns(2); c1.metric("📦 Items Shipped Today", "1,240"); c2.metric("🏭 Total Floor Assets", "4,462 Units", "+142 Returns")
...
# 🤝 MITHRA+
    ...
    if not df.empty:
        ...
        if st.button("🚀 NEGOTIATE"): st.info(f"AI generating strategy for {vendor}. Saving target: 8% cost reduction.")
