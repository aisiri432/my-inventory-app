...
# --- 5. GLOBAL TICKER ---
st.markdown(f"<div class='ticker-wrap'><div class='ticker-text'>[SYSTEM ALERT] Neural sensing active for {st.session_state.user.upper()} // [LOGISTICS] Port congestion reported +12h delay // [WEATHER] High risk to lead times in SE Asia // [MARKET] Demand sensing up 14.2%</div></div>", unsafe_allow_html=True)
...
if st.session_state.page == "Dashboard":
    ...
    with st.expander("🔔 CHETANA: Intelligent Directives", expanded=True):
        st.info("DIRECTIVE: AI recommends reordering high-value components based on rising demand sensing.")
...
# 2. PREKSHA
    ...
    if df.empty: st.warning("Add data in Nyasa.")
        ...
        with col_v:
            ...
            st.plotly_chart(px.area(y=preds, title="AI Forecasting Stream", template="plotly_dark").update_traces(line_color='#00D4FF'))
...
# 5. VITTA
    ...
    if not df.empty:
        ...
        with c2: st.plotly_chart(px.pie(df, values='current_stock', names='name', hole=0.4, template="plotly_dark"), use_container_width=True)

# 6. SANCHARA
elif st.session_state.page == "Sanchara":
    st.markdown("<div class='feature-header'>📦 SANCHARA-Ops</div>", unsafe_allow_html=True)
    t1, t2 = st.tabs(["🌐 Global Map", "Warehouse Flow"])
    with t1: st.map(pd.DataFrame({'lat': [12.97, 22.31], 'lon': [77.59, 114.16]}))
    with t2: st.metric("Floor Total", "4,320 Units")
...
# 8. NYASA
    ...
    t1, t2, t3 = st.tabs(["Manual Entry", "Bulk Sync", "PO Gen"])
    with t1:
        with st.form("add"):
