...
# --- 2. DATABASE ENGINE (WITH AUTO-REPAIR) ---
DB_FILE = 'aroha_pro_v2.db' # Renamed to force a clean start and avoid errors
...
def init_db():
    with get_db_conn() as conn:
        ...
        # Ensure Products table exists with the correct schema
...
# 3. SAMVADA
    ...
    nav_home(); st.markdown("<h2 style='color:#D4AF37; letter-spacing:4px;'>SAMVADA CHAT</h2>", unsafe_allow_html=True)
    ...
    if not key: st.error("AI Node Offline")
        ...
        p = st.chat_input("Ask Samvada...")
        if p:
            with get_db_conn() as conn:
                ctx = pd.read_sql_query("SELECT name, current_stock FROM products WHERE username=?", conn, params=(st.session_state.user,)).to_string()
            r = client.chat.completions.create(model="llama-3.1-8b-instant", messages=[{"role":"system","content":f"You are Samvada. Data: {ctx}"},{"role":"user","content":p}])
...
# 4. NYASA
    ...
    nav_home(); st.markdown("<h2 style='color:#D4AF37; letter-spacing:4px;'>NYASA LEDGER</h2>", unsafe_allow_html=True)
    with st.form("add"):
...
# 5. AGAMA (THE FIX IS HERE)
    ...
    if file:
        ...
        if all(col in u_df.columns for col in expected):
            u_df = u_df[expected].copy()
            u_df['username'] = st.session_state.user
            if st.button("SYNCHRONIZE"):
                try:
                    with get_db_conn() as conn:
                        # We use the robust connection inside the context manager
                        u_df.to_sql('products', conn, if_exists='append', index=False)
                    st.success("Synchronization Complete.")
                except Exception as e:
                    st.error(f"Sync failed: {e}")
        else:
            st.error(f"CSV missing columns. Required: {expected}")
