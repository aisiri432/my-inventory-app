...
def apply_aroha_style():
    st.markdown("""
        ...
        .sidebar-sub { font-size: 0.9rem !important; color: #6C63FF; font-weight: 700; display: block; margin-top: -10px; margin-bottom: 25px; margin-left: 20px; text-transform: uppercase; letter-spacing: 1px; }
...
# --- 5. GLOBAL TICKER & SIDEBAR ---
st.markdown(f"<div class='ticker-wrap'><div class='ticker-text'>[DHWANI] Neural link active for {st.session_state.user.upper()} // [LOGISTICS] Port congestion reported in SE Asia // [MARKET] Sentiment sensors tracking +12% Demand // [VOICE] Samvada Node Listening.</div></div>", unsafe_allow_html=True)
...
def get_data(): return get_user_data()
...
if st.session_state.page == "Dashboard":
    ...
    with st.expander("🔔 CHETANA: Live Directives", expanded=True):
        st.info("DIRECTIVE: AI recommends rebalancing inventory for high-value components due to sensed demand spikes.")

# NODE: NYASA (INPUT + SYNC + PO)
elif st.session_state.page == "Nyasa":
    st.markdown("<div class='feature-header'>NYASA</div>", unsafe_allow_html=True)
    t1, t2, t3 = st.tabs(["Manual Entry", "Bulk Ingest", "PO Generator"])
    with t1:
        with st.form("add_p"):
            n = st.text_input("Name"); c = st.text_input("Cat"); s = st.number_input("Stock", 0); p = st.number_input("Price", 0.0); lt = st.number_input("Lead", 1); sup = st.text_input("Supplier"); img = st.text_input("Img URL"); rev = st.text_area("Reviews (Split with |)")
...
# NODE: SAMVADA
    ...
    if key:
        ...
        if u_in:
            ...
            ctx = get_data().to_string(index=False)
...
# NODE: SANCHARA (MAP + FLOW + RETURNS)
    ...
    with t2:
        st.metric("Total Warehouse Units", "4,320")
    with t3:
...
# NODE: MITHRA
    ...
    st.dataframe(get_data()[['supplier', 'lead_time', 'name']], use_container_width=True)

# NODE: AGAMA
elif st.session_state.page == "Agama":
    st.markdown("<div class='feature-header'>AGAMA</div>", unsafe_allow_html=True)
    st.info("Bulk Data Sync Node. Use Nyasa node to access the Sync tab.")
