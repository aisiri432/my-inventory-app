...
if st.session_state.page == "Dashboard":
    ...
    st.markdown("<div class='insight-box'><b>💡 AI Directive:</b> All nodes synchronized. Sensed +14% demand for electronics category. Use PREKSHA for granular pathing.</div>", unsafe_allow_html=True)

# 📝 NYASA (Sync + Manual + PO)
...
# 📈 PREKSHA (AI + Images + Sentiment)
    ...
    if df.empty: st.warning("Add data in Nyasa.")
        ...
        with col_v:
            ...
            total = preds.sum()
            st.markdown(f"<div class='ai-decision-box'><h3>🤖 AGENT SUGGESTION</h3>Reorder <b>{max(0, total - p_row['current_stock'])}</b> units immediately.</div>", unsafe_allow_html=True)

# 🛡️ STAMBHA (Risk Alerts)
elif st.session_state.page == "Stambha":
    st.markdown("<div class='feature-header'>STAMBHA</div>", unsafe_allow_html=True)
    s = st.selectbox("Disruption Trigger", ["Normal", "Port Closure (3x Lead)", "Factory Fire (+30d)"])
    df = get_user_data()
    if not df.empty:
        ...
        st.table(pd.DataFrame([{"Asset": p['name'], "Survival(d)": round(p['current_stock']/12,1), "Recovery(d)": p['lead_time']*(3 if "Port" in s else 1), "Risk": "🔴" if (p['current_stock']/12) < (p['lead_time']*(3 if "Port" in s else 1)) else "🟢"} for _,p in df.iterrows()]))
...
# 💰 VITTA (Wealth + Pie)
    ...
    if not df.empty:
        ...
        with c2: st.plotly_chart(px.pie(df, values='current_stock', names='name', hole=0.5, template="plotly_dark", title="Capital Allocation Matrix"), use_container_width=True)

# 📦 SANCHARA (Map + Floor + Returns)
    ...
    with t1:
        ...
        st.markdown("<div style='background:rgba(255,255,255,0.02); padding:15px; border-radius:10px; border:1px solid #333; margin-top:15px;'>📍 Strategic Legend: Hover dots for exact physical addresses. Red zones indicate active port disruptions.</div>", unsafe_allow_html=True)
    with t2:
...
# 🎙️ SAMVADA
    ...
    if key:
        ...
        u_in = st.chat_input("Speak or type command...")
        if u_in:
...
# 🤝 MITHRA & 👷‍♂️ KRIYA
    ...
    st.warning("⚠️ Fatigue Alert: Suggested break for Station A4 (Suresh).")
