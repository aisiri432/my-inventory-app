...
# --- 5. SIDEBAR (GUARANTEED VISIBILITY) ---
with st.sidebar:
...
# STAMBHA (RISK ALERTS)
    ...
    if not df.empty:
        for _, p in df.iterrows():
            ...
            if tts < ttr: st.error(f"🔴 CRITICAL: {p['name']} stockout in {tts}d. Recovery: {ttr}d.")
...
# SANCHARA (MAP + ADDRESS)
    ...
    t1, t2 = st.tabs(["🌐 Precision Map", "↩️ Returns"])
    with t1:
...
# SAMVADA & MITHRA+
    ...
    if key:
        ...
        u_in = st.chat_input("Ask Strategic query...")
        if u_in:
