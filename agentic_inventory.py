...
if st.session_state.page == "Dashboard":
    ...
    st.markdown("<div class='insight-box'><b>💡 AI Briefing:</b> All nodes synchronized. Open <b>PREKSHA</b> for demand sensing or <b>STAMBHA</b> for stress-testing.</div>", unsafe_allow_html=True)
...
# 📈 PREKSHA (SENSING + PHOTOS + REVIEWS)
...
# 🛡️ STAMBHA (FIXED RISK ALERTS)
    ...
    s_scenario = st.selectbox("Trigger Disruption", ["Normal", "Port Closure (3x Lead)", "Factory Fire (+30d)"])
    df = get_user_data()
    if not df.empty:
        res = []
        for _, p in df.iterrows():
            ...
            res.append({"Asset": p['name'], "TTS (d)": tts, "TTR (d)": ttr, "Status": "🔴" if tts < ttr else "🟢"})
        st.table(pd.DataFrame(res))

# 👷‍♂️ KRIYA (RESTORED DIRECTIVES + FATIGUE)
elif st.session_state.page == "Kriya":
    st.markdown("<div class='feature-header'>👷‍♂️ KRIYA</div>", unsafe_allow_html=True)
    t1, t2 = st.tabs(["🏹 Directives", "📈 Shift Pulse"])
    with t1:
        st.markdown("<div class='directive-msg'><b>[DIRECTIVE]</b> Proceed to <b>Shelf B2</b>. Pick 12x Chassis.</div>", unsafe_allow_html=True)
        st.warning("⚠️ Fatigue Alert: Performance slowdown detected at Station A4.")
    with t2:
        st.metric("Shift Throughput", "142 units/hr", "↑ 12%")
        st.plotly_chart(px.line(y=[80,85,75,90,88], title="Efficiency Pulse", template="plotly_dark").update_traces(line_color='#00D4FF'), use_container_width=True)

# 🎙️ SAMVADA (RESTORED VOICE ASSISTANT)
elif st.session_state.page == "Samvada":
    st.markdown("<div class='feature-header'>🎙️ SAMVADA</div>", unsafe_allow_html=True)
    st.session_state.voice_on = st.toggle("Activate AI Voice Assistant", value=st.session_state.voice_on)
    ...
    if key:
        ...
        audio = st.audio_input("Or Speak Command")
        if audio:
...
# 💰 VITTA (RESTORED METRICS + PIE)
    ...
    if not df.empty:
        ...
        with c1:
            ...
            st.markdown(f"<div class='financial-stat' style='margin-top:20px;'>Capital Risk (15%)<br><h2 style='color:red;'>₹{total_v*0.15:,.0f}</h2></div>", unsafe_allow_html=True)
        with c2: st.plotly_chart(px.pie(df, values='current_stock', names='name', hole=0.5, template="plotly_dark", title="Allocation Matrix"), use_container_width=True)

# 📦 SANCHARA (MAP + ADDRESS + FLOW + RETURNS)
    ...
    with t1:
        map_points = pd.DataFrame({'lat':[12.97, 22.31, 1.35], 'lon':[77.59, 114.16, 103.81], 'Node':['Hub','Factory','Port'], 'Address':['MG Road, Bangalore','Lantau, HK','Singapore (🔴 CLOSED)']})
        ...
        st.markdown("<div style='background:rgba(255,255,255,0.02); padding:15px; border-radius:10px; border:1px solid #333; margin-top:15px;'>📍 Strategic Legend: Hover dots for exact physical addresses. Red indicates active port disruptions.</div>", unsafe_allow_html=True)
    with t2:
        c1, c2 = st.columns(2); c1.metric("📦 Items Shipped Today", "1,240"); c2.metric("🏭 Total Assets", f"{get_user_data()['current_stock'].sum() + 142} Units", "+142 Returns")
    with t3:
        st.table(pd.DataFrame({'Product':['Laptop','Monitor'], 'Amount':[4,2], 'Reason':['Defective Logic','Screen Bleed']}))

# 🤝 MITHRA+ (AI NEGOTIATION SCOREBOARD)
