...
if not st.session_state.logged_in:
    ...
    with col2:
        ...
        with mode[1]:
            nu = st.text_input("New Username", key="reg_u"); np = st.text_input("New Password", type="password", key="reg_p")
...
# 1. PREKSHA (Intelligence & Forecasting)
if st.session_state.page == "Preksha":
    nav_home(); st.title("🔮 Preksha Strategic Intelligence")
    col_l, col_r = st.columns([1, 2])
    with col_l:
        sentiment = st.select_slider("Market Sentiment (External)", options=[0.8, 1.0, 1.2, 1.5, 2.0], value=1.0)
    ...
    if df.empty: st.warning("No data found. Log items in Nyasa or Agama.")
    else:
        with col_r:
            ...
            fig = px.area(x=[f"Day {i+1}" for i in range(7)], y=preds, title="AI Predicted Path", template="plotly_dark")
            fig.update_traces(line_color='#D4AF37', fillcolor='rgba(212, 175, 55, 0.1)')
            st.plotly_chart(fig, use_container_width=True)
            total_pred = preds.sum()
            reorder = max(0, (total_pred + safety_buffer) - p_row['current_stock'])
            st.markdown(f"<div class='ai-suggestion'>🤖 **AI Suggestion:** Total demand: **{total_pred} units**. <br> Recommendation: Order **{reorder} units** immediately.</div>", unsafe_allow_html=True)

# 2. STAMBHA (Resilience & Stress Testing)
...
# 3. SAMVADA (Voice-to-Voice AI Assistant)
    ...
    if not key: st.error("AI Node Offline: Missing Key")
        ...
        # Voice Input
        audio_data = st.audio_input("Click and Speak to AROHA")
        if audio_data:
            with st.spinner("Listening..."):
                ...
                conn = get_db(); ctx = pd.read_sql_query("SELECT name, current_stock FROM products WHERE username=?", (st.session_state.user,), conn).to_string(); conn.close()
                res = client.chat.completions.create(model="llama-3.1-8b-instant", messages=[{"role": "system", "content": f"You are Samvada AI. Data: {ctx}. Keep answers very short."}, {"role": "user", "content": transcription}])
...
# 4. NYASA (Manual Data Ledger)
    ...
    with st.form("add_p"):
        ...
        if st.form_submit_button("LOG TO VAULT"):
            conn = get_db(); conn.execute("INSERT INTO products (username, name, current_stock, unit_price, lead_time) VALUES (?,?,?,?,?)", (st.session_state.user, n, s, p, lt)); conn.commit(); conn.close(); st.success("Asset logged successfully.")

# 5. AGAMA (Bulk Data Import)
    ...
    if file:
        ...
        if st.button("SYNCHRONIZE WITH CLOUD"):
            conn = get_db(); u_df.to_sql('products', conn, if_exists='append', index=False); conn.close(); st.success("Cloud Synchronization Complete.")
