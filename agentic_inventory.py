...
# LOGO SVG DEFINITION FOR WATERMARK
logo_svg = """
...
def apply_aroha_style():
    st.markdown(f"""
        ...
        section[data-testid="stSidebar"] .stButton > button {{ 
            ...
            font-size: 1.5rem; font-weight: 800 !important; letter-spacing: 1.5px;
        ...
        /* CARDS & PANELS */
        .saas-card {{ background: rgba(13, 17, 23, 0.8); border: 1px solid rgba(255, 255, 255, 0.05); border-radius: 12px; padding: 25px; margin-bottom: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.4); }}
...
# 👷‍♂️ KRIYA (RESTORED WORKFORCE)
    ...
    t1, t2 = st.tabs(["🏹 Missions", "📊 Team Pulse"])
    with t1:
        st.markdown("<div class='directive-msg'><b>[DIRECTIVE]</b> Proceed to <b>Shelf B2</b> via Aisle 3. Pick 12x Chassis.</div>", unsafe_allow_html=True)
        st.warning("⚠️ Fatigue Alert: Performance slowdown detected at Station A4.")
    with t2:
        st.metric("Avg Throughput", "142 units/hr", "↑ 12%")
        fig = go.Figure(data=go.Scatterpolar(r=[90, 80, 70, 95], theta=['Speed','Accuracy','Packing','Safety'], fill='toself'))
        st.plotly_chart(fig.update_layout(template="plotly_dark", showlegend=False), use_container_width=True)

# 🤝 MITHRA+ (AI NEGOTIATOR & SCOREBOARD)
    ...
    df = get_user_data()
    t1, t2 = st.tabs(["🚀 AI Negotiation", "🏅 Scoreboard"])
    with t1:
        if not df.empty:
...
# 📦 SANCHARA (MAP + DESCRIPTION + FLOOR + RETURNS)
