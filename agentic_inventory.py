...
def apply_aroha_style():
    # Using DOUBLE BRACES {{ }} for CSS to prevent the f-string SyntaxError
    st.markdown(f"""
        <style>
...
# --- 2. DATABASE ---
DB_FILE = 'aroha_ultimate_v116.db'
...
# --- 5. TOP HUD ---
st.markdown(f"<div class='ticker-wrap'><div class='ticker-text'>[DHWANI] {st.session_state.user.upper()} ACTIVE // [LOGISTICS] Hover Map for Precision Addresses // [MARKET] Demand Sensing active (+12%) // [SYSTEM] Vault Encrypted.</div></div>", unsafe_allow_html=True)
...
if st.session_state.page == "Dashboard":
    ...
    st.markdown("<div class='insight-box'><b>💡 AI Directive:</b> System link stable. Import warehouse assets in NYASA to begin autonomous sensing.</div>", unsafe_allow_html=True)
...
# 📈 PREKSHA (Photos + Reviews + Decisions)
    ...
    if df.empty: st.warning("Add data in NYASA.")
        ...
        with col_v:
            ...
            st.markdown(f"<div class='ai-decision-box'><h3>🤖 AI SUGGESTION</h3>Reorder <b>{max(0, preds.sum() - p_row['current_stock'])}</b> units now to maintain safety buffer.</div>", unsafe_allow_html=True)
