...
# --- 5. TOP TICKER ---
st.markdown(f"<div class='ticker-wrap'><div class='ticker-text'>[SYSTEM] Neural link active for {st.session_state.user.upper()} // [LOGISTICS] Port congestion reported in SE Asia Hub // [MARKET] Sentiment sensors tracking +12% Demand // [VOICE] Samvada Node Listening.</div></div>", unsafe_allow_html=True)
...
# 2. NYASA (MANUAL + BULK + PO)
    ...
    with t1:
        with st.form("add"):
            n = st.text_input("Product Name"); c = st.text_input("Category"); s = st.number_input("Stock", 0); p = st.number_input("Price", 0.0); lt = st.number_input("Lead Time", 1); sup = st.text_input("Supplier"); img = st.text_input("Image URL"); rev = st.text_area("Reviews")
            if st.form_submit_button("Commit"):
                ...
                st.success("Asset Synchronized.")
    with t2:
        f = st.file_uploader("Upload CSV Supply Ledger", type="csv")
        if f and st.button("Batch Ingest"):
...
# 3. PREKSHA
    ...
    if df.empty: st.warning("Treasury empty. Use NYASA.")
        ...
        target = st.selectbox("Select Asset", df['name']); p_row = df[df['name'] == target].iloc[0]
        ...
        with col_v:
            ...
            Decision: Reorder <b>{max(0, total_needed - p_row['current_stock'])}</b> units immediately to satisfy demand flow.</div>""", unsafe_allow_html=True)
...
# 7. SANCHARA
    ...
    st.metric("Total Warehouse Flow", "4,320 Units Present")
...
# 8. MITHRA & AGAMA
    ...
    st.info("Bulk Data Sync Node. Use Nyasa for CSV Upload.")
