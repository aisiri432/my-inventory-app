...
# --- 5. TOP TICKER ---
st.markdown(f"<div class='ticker-wrap'><div class='ticker-text'>[DHWANI] Neural link active for {st.session_state.user.upper()} // [LOGISTICS] Hover over Map for Precision Addresses // [SENSING] Predicted +12% demand spike.</div></div>", unsafe_allow_html=True)
...
if st.session_state.page == "Dashboard":
    ...
    st.markdown("<div class='insight-box'><b>💡 User Insight:</b> Use NYASA to import data. AROHA will then sense demand spikes in PREKSHA and risk alerts in STAMBHA.</div>", unsafe_allow_html=True)

# 📦 SANCHARA (ALL FEATURES: MAP + DESCRIPTION + FLOW + RETURNS)
elif st.session_state.page == "Sanchara":
    st.markdown("<div class='feature-header'>📦 SANCHARA</div>", unsafe_allow_html=True)
    st.markdown("<div class='insight-box'><b>🚀 Intelligence Briefing:</b> Monitoring 4 global hubs. Transit flow is stable with minor congestion in Singapore.</div>", unsafe_allow_html=True)
    
    t1, t2, t3 = st.tabs(["🌐 Precision Map", "📦 Floor Ops", "↩️ Returns (PUNAH)"])
    
    df_returns = pd.DataFrame({'Product': ['Quantum Laptop', '4K Monitor', 'Keyboard'], 'Amount': [4, 2, 15], 'Reason': ['Defective Logic', 'Screen Bleed', 'Transit Damage']})

    with t1:
        st.subheader("Global SPANDANA Map")
        map_points = pd.DataFrame({'lat':[12.97, 22.31, 37.77, 1.35], 'lon':[77.59, 114.16, -122.41, 103.81], 'Node':['Bangalore Hub', 'HK Factory', 'SF HQ', 'Port'], 'Address':['MG Road, Bangalore, India', 'Lantau, HK', 'Market St, SF', 'Jurong, Singapore (🔴 PORT CLOSED)']})
        ...
        st.markdown("<div style='background:rgba(255,255,255,0.02); padding:15px; border-radius:10px; border:1px solid #333; margin-top:15px;'><h4 style='color:#00D4FF; margin-top:0;'>🗺️ Strategic Map Description</h4><p>📍 Blue Nodes: Verified supplier hubs. 🔴 Red Markers: Global disruption zones. Tap markers for exact physical addresses.</p></div>", unsafe_allow_html=True)
    with t2:
        st.subheader("Warehouse Throughput")
        c1, c2, c3 = st.columns(3)
        ...
        c3.metric("🏭 Total Floor Assets", f"{current_sum + df_returns['Amount'].sum()} Units", f"+{df_returns['Amount'].sum()} Returns")
    with t3:
        st.subheader("PUNAH Returns Ledger")
        st.dataframe(df_returns, use_container_width=True)
        st.plotly_chart(px.pie(df_returns, values='Amount', names='Reason', hole=0.5, template="plotly_dark"))

# 🛡️ STAMBHA (FIXED RISK ALERTS)
    ...
    if not df.empty:
        for _, p in df.iterrows():
            ...
            if tts < ttr: st.error(f"🔴 CRITICAL ALERT: {p['name']} runs out in {tts}d. Recovery takes {ttr}d. STOCKOUT RISK.")
        st.table(pd.DataFrame([{"Asset": p['name'], "TTS (d)": round(p['current_stock']/12,1), "Risk": "🔴" if (p['current_stock']/12) < (p['lead_time']*(3 if "Port" in s else 1)) else "🟢"} for _,p in df.iterrows()]))

# 📈 PREKSHA (REVIEWS + PHOTOS + DECISIONS)
...
# 💰 VITTA
    ...
    if not df.empty:
        ...
        with c1:
            ...
            st.markdown(f"<div class='financial-stat' style='margin-top:20px;'>Capital Risk<br><h2 style='color:red;'>₹{total_v*0.15:,.0f}</h2></div>", unsafe_allow_html=True)
        with c2: st.plotly_chart(px.pie(df, values='current_stock', names='name', hole=0.5, template="plotly_dark"))

# 📝 NYASA (PO GEN INCLUDED)
    ...
    with t3:
        df = get_user_data()
        if not df.empty:
            t = st.selectbox("Select for PO", df['name'])
