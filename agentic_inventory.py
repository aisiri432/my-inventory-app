...
# --- 5. TICKER ---
st.markdown(f"<div class='ticker-wrap'><div class='ticker-text'>[SYSTEM ALERT] Neural sensing active for {st.session_state.user.upper()} // [RISK] Port closure detected in Disruption Node // [STRATEGY] AI suggests immediate restock for critical assets.</div></div>", unsafe_allow_html=True)

# --- 6. SIDEBAR (RE-ORDERED: NYASA MOVED TO START) ---
with st.sidebar:
    ...
    # Re-ordered list
    nodes = [
        ("NYASA", "Nyasa", "Log Assets Securely"), # MOVED TO START
...
# 3. PREKSHA (Demand Sensing + SUGGESTIONS RESTORED)
    ...
    if df.empty: st.warning("Add data in Nyasa.")
        ...
        with col_v:
            sent = st.select_slider("Market Sensing", options=[0.8, 1.0, 1.2, 1.5, 2.0], value=1.0)
...
# 4. STAMBHA (Resilience + RISK MESSAGES RESTORED)
    ...
    if not df.empty:
        ...
        for _, p in df.iterrows():
            ...
            # --- RESTORED RISK MESSAGES ---
            if status == "🔴 CRITICAL":
                st.error(f"⚠️ DANGER: {p['name']} will run out in {tts} days. Recovery time is {ttr} days. STOCKOUT IMMINENT.")
...
# 7. SANCHARA & MITHRA
    ...
    st.metric("Warehouse Flow Today", "154 Units Shipped")
