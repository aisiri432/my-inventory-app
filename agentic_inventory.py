...
# --- 1. PREMIUM UI CONFIG (UNIVERSAL RESPONSIVE & RADIANT GLOW) ---
st.set_page_config(
...
def apply_aroha_style():
    st.markdown(f"""
        ...
        @media (min-width: 768px) {{
            ...
            .brand-title {{ font-size: 3.8rem !important; }}
        ...
        /* 💎 RADIANT GLOW BRANDING */
        .brand-title {{ 
            font-weight: 800; color: #FFFFFF; letter-spacing: -2px; margin-bottom: 0; 
            text-shadow: 0 0 15px #00D4FF, 0 0 30px #6C63FF; /* RADIANT GLOW */
        }}
        ...
        with m[1]:
            ...
            if st.button("Enroll Session"):
                try:
                    ...
                    st.success("Authorized! Now Switch to Login tab.")
                except: st.error("ID exists.")
    st.stop()

# --- 5. AUTHORIZED INTERFACE (SIDEBAR + TICKER) ---
st.markdown(f"<div class='ticker-wrap'><div class='ticker-text'>[DHWANI] {st.session_state.user.upper()} ACTIVE // [LOGISTICS] Port congestion reported in SE Asia // [MARKET] Predicted +12% demand spike.</div></div>", unsafe_allow_html=True)
...
if st.session_state.page == "Dashboard":
    ...
    st.markdown("<div class='insight-box'><b>💡 Tactical Insight:</b> System sensing active. AROHA recommends verifying lead times in MITHRA+ due to Suez Canal delays.</div>", unsafe_allow_html=True)

# 👷‍♂️ KRIYA (RESTORED WORKFORCE SPEC)
elif st.session_state.page == "Kriya":
    st.markdown("<div class='feature-header'>KRIYA</div>", unsafe_allow_html=True)
    st.markdown("<div class='insight-box'><b>🚀 Performance Insight:</b> Workforce efficiency is up 12%. No critical fatigue detected in current shift.</div>", unsafe_allow_html=True)
    t1, t2 = st.tabs(["🏹 Missions", "📊 Analytics Radar"])
    with t1:
        ...
        if st.button("📦 SIMULATE ERROR SCAN"):
            st.error("🚨 [ERROR PREVENTION] Warning: Incorrect item selected (SKU-405). Please verify SKU on Bin B2.")
        st.warning("⚠️ Fatigue Alert: Performance slowdown (15%) sensed at Station A4.")
    with t2:
        fig = go.Figure(data=go.Scatterpolar(r=[90, 80, 70, 95, 85], theta=['Speed','Accuracy','Packing','Safety','Sync'], fill='toself'))
...
# 💰 VITTA (RESTORED METRICS + PIE)
    ...
    if not df.empty:
        ...
        with c1:
            ...
            st.info("💡 Insight: High capital lock-in detected. Suggest bulk-discount sale for accessories.")
...
# 📦 SANCHARA (MAP + ADDRESS + RETURNS RESTORED)
    ...
    with t1:
        map_points = pd.DataFrame({'lat':[12.9716, 22.31, 37.77, 1.35], 'lon':[77.59, 114.16, -122.41, 103.81], 'Node':['Hub','Factory','HQ','Port'], 'Address':['MG Road, Bangalore','Lantau, HK','Market St, SF','Jurong, Singapore (🔴 PORT CLOSED)']})
    ...
    with t3:
        st.subheader("PUNAH Returns Reasoning Analysis")
        df_ret = pd.DataFrame({'Product':['Laptop', 'Monitor', 'Cables'], 'Amount':[4, 2, 120], 'Reason':['Defective Logic', 'Screen Bleed', 'Wrong Color']})
        st.table(df_ret)
        st.info("💡 Decision Insight: Defective Logic boards in Laptops require immediate supplier audit (MITHRA+).")

# 📈 PREKSHA (PHOTOS + REVIEWS + SUGGESTIONS RESTORED)
    ...
    if not df.empty:
        ...
        with col_m:
            ...
            if p['reviews'] and str(p['reviews']) != "nan":
                st.subheader("Verified Sentiment")
        ...
        with col_v:
            ...
            st.markdown(f"<div class='ai-decision-box'><h3>🤖 AI SUGGESTION</h3>Reorder <b>{max(0, preds.sum() - p['current_stock'])}</b> units now to maintain safety buffer.</div>", unsafe_allow_html=True)

# 🛡️ STAMBHA (RISK MESSAGES RESTORED)
elif st.session_state.page == "Stambha":
    st.markdown("<div class='feature-header'>STAMBHA</div>", unsafe_allow_html=True)
    s = st.selectbox("Disruption Event", ["Normal", "Port Closure (3x Lead)", "Factory Fire (+30d)"])
    df = get_user_data()
    if not df.empty:
        for _, p in df.iterrows():
            ttr = p['lead_time'] * (3 if "Port" in s else 1)
            tts = round(p['current_stock'] / 12, 1)
            if tts < ttr: st.error(f"🔴 CRITICAL RISK: {p['name']} runs out in {tts}d. Recovery takes {ttr}d. IMMINENT STOCKOUT.")
        st.table(df[['name', 'current_stock', 'lead_time']])

# SAMVADA & MITHRA+ (PRESERVATION)
    ...
    if not df.empty:
        ...
        if st.button("🚀 INITIATE NEGOTIATION"): st.info(f"AI Strategy: Volume leverage identified for {vendor}. Target 8% discount.")
